...
class MCTSNode(TreeNode):
    ...
    def expand(self, getAction, getReward, max_a, step):
...
class MCTSTree(Tree):
    def __init__(self, getAction, getReward, max_w = 3, step = 5 , budget = 50, bp_policy = "max"):
    ...
    def search(self):
        ...
        for b in tqdm(range(self.budget, 0, -1), desc="MCTS searching"):
            ...
            node, reward = node.expand(self.getAction, self.getReward, max_a, self.step)
            if reward == 1.0:
